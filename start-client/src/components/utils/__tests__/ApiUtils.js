...
describe('getShareUrl', () => {
  it('return the base URL with custom values', () => {
    ...
    expect(result).toBe(
      'type=foo1&language=foo2&platformVersion=foo3&packaging=foo4&jvmVersion=foo5&groupId=foo6&artifactId=foo7&name=foo8&description=foo9&packageName=foo10&dockerizable=true&dependencies=foo11,foo12'
    )
  })
})
...
describe('getProject', () => {
  ...
  it('should call the right service with the right parameters', () => {
    ...
    expect(fetch.mock.calls[0][0]).toEqual(
      'http://demo/starter.zip?type=foo1&language=foo2&bootVersion=foo3&baseDir=foo7&groupId=foo6&artifactId=foo7&name=foo8&description=foo9&packageName=foo10&packaging=foo4&javaVersion=foo5&dockerizable=true&dependencies=foo11,foo12'
    )
  })
  ...
  it('should call the right service with the right parameters (clean invalid dependencies)', () => {
    ...
    expect(fetch.mock.calls.length).toEqual(1)
    expect(fetch.mock.calls[0][0]).toEqual(
      'http://demo/starter.zip?type=foo1&language=foo2&bootVersion=foo3&baseDir=foo7&groupId=foo6&artifactId=foo7&name=foo8&description=foo9&packageName=foo10&packaging=foo4&javaVersion=foo5&dockerizable=true&dependencies=foo11'
    )
  })
  ...
  it('should call the right service with the right parameters (no dependency)', () => {
    ...
    expect(fetch.mock.calls.length).toEqual(1)
    expect(fetch.mock.calls[0][0]).toEqual(
      'http://demo/starter.zip?type=foo1&language=foo2&bootVersion=foo3&baseDir=foo7&groupId=foo6&artifactId=foo7&name=foo8&description=foo9&packageName=foo10&packaging=foo4&javaVersion=foo5&dockerizable=true'
    )
  })
})
